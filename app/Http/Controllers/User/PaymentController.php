...
class PaymentController extends Controller
{
    ...
    public function paymentpointTopUp(){
        $page_title = 'Payment Confirm';

        $bankAccount = optional(auth()->user())->bankAccount;
        $walletBalance = optional(auth()->user())->balance;

        $gateway = Gateway::findOrFail("43");

        return view(template() . "user.payment.paymentpoint", compact('page_title', 'bankAccount', 'gateway', 'walletBalance'));
    }
    ...
    public function depositConfirm($trx_id)
    {
        try {
            ...
            if ($deposit->child_panel_id ? 999 < $gateway->gateway_id : 999 < $gateway->id) {
                ...
                return view(template() . 'user.payment.manual', compact('deposit', 'walletBalance'));
            }
            ...
            if ($childPanel) {
                return view(childPanelTemplate() . $data->view, compact('data', 'page_title', 'deposit', 'walletBalance'));
            }
            return view(template() . $data->view, compact('data', 'page_title', 'deposit', 'walletBalance'));
    ...
    public function fromSubmit(Request $request, $trx_id)
    {
        ...
        return redirect()->route('user.add.fund');
    }
